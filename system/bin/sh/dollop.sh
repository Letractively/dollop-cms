#!/usr/bin/env php
<?php
// $Id: dollop.sh 3 2013-02-03 11:57:44Z fire1.A.Zaprianov@gmail.com $

/**
 * dollop shell execution script
 *
 * @param path  Dollop's absolute root directory .
 * @param URI   A URI to execute (including HTTP protocol prefix).
 *
 * For Windows
 * replace line 1 (first line) with
...
if (in_array('--help', $_SERVER['argv']) || empty($_SERVER['argv'])) {
  echo <<<EOF
Execute a Dollop page from the shell.
Usage:        {$script} [OPTIONS] "<URI>"
Example:      {$script} "http://mysite.com/node"
All arguments are long options.

  --help      This page.

  --root      Set the working directory for the script to the specified path.
              To execute dollop this has to be the root directory of your
              dollop installation, f.e. /home/www/foo/dollop (assuming dollop
  ...
  URI         The URI to execute, i.e. http://default/foo/bar for executing
              ...
              and the domain must exist in dollop's sites-directory.
              ...
              bootstrapping dollop. To execute dollop's cron.php, specify
              http://default/cron.php as the URI.


To run this script without --root argument invoke it from the root directory
of your Dollop installation with
  ...
  switch ($param) {
    ...
    default:
      if (substr($param, 0, 2) == '--') {
        ...
        // set file to execute or dollop path (clean urls enabled)
